...
def fetch_fundamentals(tickers: list[str], progress=None) -> pd.DataFrame:
    """One row per ticker of headline fundamentals. Missing fields become NaN."""
    import yfinance as yf

    rows = {}
    for i, ticker in enumerate(tickers):
        if progress:
            progress(i / max(len(tickers), 1), ticker)
        info = {}
        try:
        ...
        pe = _num(info.get("trailingPE")) or np.nan
        ...
        rows[ticker] = {
            ...
            "pe": pe,
            "earnings_yield": _safe_div(1, pe),
            "fcf_yield": _safe_div(_num(info.get("freeCashflow")), market_cap),
            "ev_ebitda": _safe_div(_num(info.get("enterpriseValue")), ebitda),
            ...
            "net_debt_ebitda": _safe_div(total_debt - cash, ebitda)
            if not (np.isnan(total_debt) or np.isnan(cash))
            else np.nan,
...
def build_snapshot(tickers: list[str], progress=None) -> tuple[pd.DataFrame, pd.DataFrame]:
    prices = fetch_prices(tickers)
    fundamentals = fetch_fundamentals(tickers, progress=progress)
    features = price_features(prices) if not prices.empty else pd.DataFrame()
    if not features.empty:
        fundamentals = fundamentals.join(features, how="left")
    # yfinance reports debt/equity in percent
    if "debt_to_equity" in fundamentals:
        fundamentals["debt_to_equity"] = pd.to_numeric(
            fundamentals["debt_to_equity"], errors="coerce"
        ) / 100
