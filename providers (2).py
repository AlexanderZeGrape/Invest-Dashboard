...
def sec_profile(cik: str) -> tuple[str, str]:
    """(company name, sector) from the submissions endpoint. companyfacts has no sector."""
    if cik in _PROFILE_CACHE:
        return _PROFILE_CACHE[cik]
    name, sector = "", "Unknown"
    try:
        payload = requests.get(f"https://data.sec.gov/submissions/CIK{cik}.json",
                               headers=HEADERS, timeout=TIMEOUT).json()
        name = payload.get("name", "") or ""
        sector = SIC_SECTORS.get(str(payload.get("sic", ""))[:2],
                                 payload.get("sicDescription") or "Unknown")
    except Exception:
        pass
    _PROFILE_CACHE[cik] = (name, sector)
    return name, sector


# First two digits of the SEC SIC code -> a readable sector.
SIC_SECTORS = {
    "01": "Agriculture", "10": "Mining", "12": "Mining", "13": "Energy",
    "14": "Mining", "15": "Construction", "16": "Construction",
    "17": "Construction", "20": "Consumer staples", "21": "Consumer staples",
    "22": "Textiles", "23": "Textiles", "24": "Materials", "25": "Consumer goods",
    "26": "Materials", "27": "Media", "28": "Pharma & chemicals",
    "29": "Energy", "30": "Materials", "31": "Consumer goods", "32": "Materials",
    "33": "Materials", "34": "Industrials", "35": "Industrials",
    "36": "Electronics", "37": "Industrials", "38": "Instruments & semis",
    "39": "Industrials", "40": "Transport", "42": "Transport", "44": "Transport",
    "45": "Transport", "48": "Telecom", "49": "Utilities", "50": "Wholesale",
    "51": "Wholesale", "52": "Retail", "53": "Retail", "54": "Retail",
    "55": "Retail", "56": "Retail", "57": "Retail", "58": "Restaurants",
    "59": "Retail", "60": "Banks", "61": "Financials", "62": "Financials",
    "63": "Insurance", "64": "Insurance", "65": "Real estate",
    "67": "Financials", "70": "Hotels", "72": "Services", "73": "Software & IT",
    "78": "Media", "79": "Leisure", "80": "Healthcare", "82": "Education",
    "83": "Services", "87": "Professional services", "99": "Other",
}


def _annual_values(facts: dict, tags: list[str], namespace: str = "us-gaap",
                   unit: str = "USD", strict: bool = True) -> tuple[float, float]:
    ...
    for tag in tags:
        ...
        for unit_key in (unit, "USD/shares", "shares", "pure"):
            ...
            if strict:
                ...
                annual = [r for r in rows if r.get("val") is not None]
            if len(annual) < 1:
                continue
...
def sec_fundamentals(tickers: list[str], prices: dict[str, float] | None = None,
    ...
    for i, ticker in enumerate(tickers):
        ...
        sec_name, sector = sec_profile(cik)
        record["name"] = sec_name or facts.get("entityName", ticker)
        ...
        for field in ("market_cap", "pe", "price_to_book", "ev_ebitda"):
