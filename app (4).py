...
import providers
from config import (CACHE_HOURS, DEFAULT_WEIGHTS, FACTOR_MAP, METRIC_LABELS,
                    UNIVERSES, universe_class, universe_tickers)
...
def secret(name: str) -> str:
    """st.secrets raises if no secrets file exists — treat that as 'no key'."""
    try:
...
fmp_key = ""
finnhub_key = secret("FINNHUB_KEY")
if not demo_mode:
    with st.sidebar.expander("Data sources", expanded=False):
        price_source = st.selectbox("Prices", ["Stooq", "Yahoo"])
        fundamental_source = st.selectbox(
            "Fundamentals",
            ["SEC EDGAR (US only)", "FMP (global)", "Yahoo"],
            help="SEC needs no key but only covers US filers. FMP covers Xetra "
                 "and Euronext but needs a free API key.",
        )
        fmp_key = secret("FMP_KEY")
        if fundamental_source == "FMP (global)" and not fmp_key:
            fmp_key = st.text_input("FMP API key", type="password")
        if not finnhub_key:
            finnhub_key = st.text_input("Finnhub key (news, optional)",
                                        type="password")
...
def get_data(force: bool = False):
    ...
    if force or stale:
        ...
        try:
            snapshot, prices = dl.build_snapshot(
                tickers, price_source=price_source,
                fundamental_source=fundamental_source, fmp_key=fmp_key,
                asset_class=asset_class, progress=report,
            )
...
is_macro = (not demo_mode) and asset_class == "macro"

if is_macro:
    table = snapshot.copy()
    for column in ("Score", "Coverage"):
        table[column] = np.nan
    for factor in FACTORS:
        table[factor] = np.nan
else:
...
if not demo_mode and not is_macro and table["Coverage"].fillna(0).max() < 0.05:
    st.error(
        "The data provider returned no fundamentals for any ticker — every field is empty. "
        "This is usually Yahoo blocking requests from cloud servers, not a problem with your "
        "setup. Try **Fetch fresh data** again in a few minutes; if it keeps failing, the app "
        "needs a data source that allows server access."
    )

filtered = table
mask = pd.Series(True, index=table.index)
if (not is_macro) and min_cap > 0 and "market_cap" in table:
    mask &= table["market_cap"].fillna(0) >= min_cap
if (not is_macro) and profitable_only and "pe" in table:
    mask &= table["pe"] > 0
if not is_macro:
...
# tabs
# --------------------------------------------------------------------------
(screen_tab, news_tab, company_tab, value_tab, portfolio_tab, journal_tab,
 method_tab) = st.tabs(
    ["Screen", "News", "Company", "Valuation", "Portfolio", "Journal", "Method"]
)
...
with screen_tab:
    ...
    if is_macro:
        display_cols = ["price", "mom_12_1", "mom_6_1", "above_200d",
    ...
    if is_macro:
        ...
        with macro_left:
            st.caption("Performance, rebased to 100 (last 12 months)")
            window = prices.tail(252).dropna(axis=1, how="all")
            if window.empty:
                st.info("No price history cached.")
            else:
                rebased = window / window.bfill().iloc[0] * 100
                line = go.Figure()
                for column in rebased.columns:
                    line.add_trace(go.Scatter(x=rebased.index, y=rebased[column],
                                              name=column, line=dict(width=1.2)))
                line.update_layout(margin=dict(l=10, r=10, t=10, b=10), height=420)
                wide(st.plotly_chart, line)
        with macro_right:
            st.caption("Correlation of daily returns (last 12 months)")
            returns = prices.tail(252).pct_change().dropna(how="all")
            corr = returns.corr()
            if corr.empty:
                ...
                st.caption(
                    "Blue = move together, red = move opposite. These shift over "
                    "time, so treat one snapshot as a hypothesis, not a fact."
                )
        st.stop()
    ...
    with chart_left:
        ...
        scatter_source = scatter_source.dropna(subset=["Value", "Quality"])
        scatter_source["market_cap"] = (
            pd.to_numeric(scatter_source.get("market_cap"), errors="coerce")
            .fillna(1.0).clip(lower=0.1)
        )
        if scatter_source.empty:
            st.info("Nothing to plot — no company has both a Value and a Quality score.")
        else:
            fig = px.scatter(
                scatter_source,
    ...
    with chart_right:
        ...
        if by_sector.empty or by_sector.isna().all().all():
...
def load_news(feeds: tuple[str, ...]):
    return newsfeed.fetch_feeds(list(feeds))
...
with news_tab:
    ...
    with head_left:
        st.subheader("Squawk")
        st.markdown(
            "<div class='desk-note'>Merged headlines from several free feeds, newest "
            "first. Tags are keyword-derived, so treat them as a rough filter rather "
            "than a classification.</div>",
            unsafe_allow_html=True,
        )
    with head_right:
    ...
    chosen_feeds = st.multiselect(
        "Sources", list(newsfeed.FEEDS),
        default=["Breaking", "Economy", "Commodities", "Forex", "ForexLive"],
    )

    with st.spinner("Pulling headlines…"):
        stream = load_news(tuple(chosen_feeds))

    if stream.empty:
        st.warning(
            "No headlines came back. The feeds are public RSS, so this is usually a "
            "temporary network issue — press Refresh feed in a moment."
        )
    else:
        control_left, control_mid, control_right = st.columns([2, 1, 1])
        with control_left:
            chosen_tags = st.multiselect("Filter by tag", list(newsfeed.TAGS))
        with control_mid:
            impact_only = st.checkbox("High impact only")
        with control_right:
            mine_only = st.checkbox("Mentions my universe")

        search = st.text_input("Search headlines", placeholder="opec, cpi, micron…")

        view = stream
        if chosen_tags:
            view = view[view["tags"].apply(
                lambda tags: any(tag in tags for tag in chosen_tags))]
        if impact_only:
            view = view[view["impact"]]
        if mine_only:
            company_names = (snapshot["name"].dropna().tolist()
                             if "name" in snapshot.columns else [])
            view = view[newsfeed.match_universe(view, tickers, company_names)]
        if search.strip():
            view = view[view["headline"].str.contains(search.strip(), case=False,
                                                      na=False)]

        st.caption(f"{len(view)} of {len(stream)} headlines")
        now = pd.Timestamp.now(tz="UTC")
        for item in view.head(80).itertuples():
            stamp = "—"
            if pd.notna(item.published):
                minutes = (now - item.published).total_seconds() / 60
                if minutes < 60:
                    stamp = f"{int(max(minutes, 0))}m"
                elif minutes < 1440:
                    stamp = f"{int(minutes // 60)}h"
                else:
                    stamp = item.published.strftime("%d %b")
            flag = "**!**  " if item.impact else ""
            tags = " · ".join(item.tags)
            st.markdown(
                f"`{stamp:>4}`  {flag}[{item.headline}]({item.url})  \n"
                f"<span class='desk-note'>{item.source}"
...
with company_tab:
    ...
    if not options:
        ...
        if finnhub_key and not demo_mode:
            st.divider()
            st.caption("Recent headlines")
            headlines = providers.finnhub_news(ticker, finnhub_key)
            if not headlines:
                st.caption("No headlines returned for this ticker.")
            for item in headlines:
                stamp = item["datetime"]
                stamp = stamp.strftime("%d %b") if pd.notna(stamp) else ""
                st.markdown(
                    f"- [{item['headline']}]({item['url']}) · {item['source']} {stamp}"
                )
