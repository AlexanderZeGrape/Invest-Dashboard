...
Set in the sidebar under **Data sources** (live mode only).

| Layer | Options | Key needed |
|---|---|---|
| Prices | **Stooq** (default, works from cloud servers), Yahoo | No |
| Fundamentals | **SEC EDGAR** (US filers, official), **FMP** (global incl. Xetra/Euronext), Yahoo | SEC no, FMP yes |
| News | Finnhub, shown per company | Yes |

Stooq covers US, German and most European listings and needs no signup. Tickers
that Stooq doesn't have fall back to Yahoo automatically.

Before using the SEC source, open `providers.py` and put a real email address in
`CONTACT` — the SEC rejects requests without a contact in the User-Agent.

API keys go in Streamlit secrets, never in the code. On Community Cloud:
...
Locally the same lines go in `.streamlit/secrets.toml` (git-ignore that file).

## Universes

Equity universes (AI & compute, Memory & semis, Precious metals, Energy, DAX 40,
US large caps) get the full factor score. Macro universes (Commodities & macro,
ETF allocation) hold ETFs, indices and commodity proxies — they have no
fundamentals, so they show price behaviour, a rebased performance chart and a
correlation matrix instead of a ranking.

## Files

```
config.py      universes, factor definitions, default weights   ← edit this first
providers.py   Stooq / SEC EDGAR / FMP / Yahoo / Finnhub adapters
data.py        fetch orchestration, SQLite cache, demo generator
